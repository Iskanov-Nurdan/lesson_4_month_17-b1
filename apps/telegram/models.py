...
class Telegram(models.Model):
    ...
    persone = models.CharField(
        max_length=20, choices=PERSONE_CHOISE, verbose_name="Количество людей"
    )
    date = models.DateField(verbose_name="Дата")
    time = models.CharField(max_length=20, choices=TIME_CHOISE, verbose_name="Время")
...
class TelegramUser(models.Model):
    ...
    username = models.CharField(
        max_length=255, verbose_name="Имя пользователя", blank=True, null=True
    )
    first_name = models.CharField(
        max_length=255, verbose_name="Имя", blank=True, null=True
    )
    last_name = models.CharField(
        max_length=255, verbose_name="Фамилия", blank=True, null=True
    )
